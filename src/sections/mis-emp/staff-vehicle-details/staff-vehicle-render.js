...
import StaffVehicleForm from './staff-vehicle-form';
import SalesDetailsForm from '../sales-details-form';
import SearchBar from 'src/components/search/Search-bar';
import EditStaffVehicleForm from './edit-staff-vehicle-form';
import ShowStaffVehicleBillDetails from './show-staff-vehicle-details';
...
export default function StaffVehicleRender({ title, subheader, tableLabels, tableData, ...other }) {
        ...
        <Container maxWidth={settings.themeStretch ? false : 'xl'}>
            <Card {...other}>
                ...
                <SearchBar
                    ...
                    placeholder='Search with Vehicle, Company, Division, Deparment'
                />
                    ...
                    >
                        <Box sx={{ p: 3, borderBottom: `solid 1px ${theme.palette.divider}` }}>
                            <InputBase
                                fullWidth
                                autoFocus
                                placeholder="Add Staff Vehicle Details"
                                value={''}
                                onChange={() => {
                                    '';
                                }}
                        ...
                        <Scrollbar sx={{ p: 3, pt: 2, height: 400 }}>{renderStaffVehicleForm()}</Scrollbar>
                    </Dialog>
                        ...
                        <Table sx={{ minWidth: 960 }}>
                            ...
                            <TableBody>
                                {dataFiltered ? dataFiltered.slice(
                                    ...
                                    <StaffVehicleRow key={row.id} row={row} />
                                )) : <LinearProgress />}
                            </TableBody>
                        </Table>
                    </Scrollbar>
...
function StaffVehicleRow({ row }) {
    ...
    const handleDelete = async(gotId) => {
        try{
            const URL = `http://localhost:5000/api/v1/bill/makeAndModel/${gotId}`
          ...
          >
            <Box sx={{ p: 3, borderBottom: `solid 1px ${theme.palette.divider}` }}>
              <InputBase
                fullWidth
                autoFocus
                placeholder="Detail of Staff Vehicle Bill"
                value={''}
                onChange={() => {
                  '';
                }}

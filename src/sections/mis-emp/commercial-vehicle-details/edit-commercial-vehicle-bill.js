...
export default function EditCommercialVehicleForm({ currentUser }) {
    ...
    const CommercialVehicleSchema = Yup.object().shape({
        ...
        driver: Yup.string().required('Driver details are required'),
    })
    ...
    const methods = useForm({
        resolver: yupResolver(CommercialVehicleSchema),
        defaultValues,
    });
    ...
    const getAllVehicles = async() => {
        try{
            ...
            const res = await axios.get(URL);
            const generetedData = res?.data?.data.map((el) => {
                return { label: el.vehicle_name, value: el._id }
            })
    ...
    const getALlDrivers = async() => {
        try{
            const URL = "http://localhost:5000/api/v1/driver/getAll"
            const res = await axios.get(URL);
            const generetedData = res?.data?.data.map((el) => {
                return { label: el.first_name, value: el._id }
            })

            setAllDrivers(generetedData);

        }
        catch(err){
            alert("something went wrong in get all Drivers");
            console.log(err)
        }
    }
    ...
    const getCommercialVehicleBill = async() => {
        try{
            const URL = `http://localhost:5000/api/v1/bill/vehicle/${itemMasterId}`
            const res = await axios.get(URL)
            setValue('makeModel', res.data.data.make_model, { shouldValidate: true });
            setValue('vehicleNo', res.data.data.vehicle_no, { shouldValidate: true });
            setValue('purchaseDate', dayjs(res.data.data.purchase_date), { shouldValidate: true });
            setValue('issueDate', dayjs(res.data.data.issue_date), { shouldValidate: true });
            setValue('recievedDate', dayjs(res.data.data.received_date), { shouldValidate: true });
            setValue('assetNo', res.data.data.asset_no, { shouldValidate: true });
            setValue('chassisNo', res.data.data.chassis_no, { shouldValidate: true });
            setValue('engineNo', res.data.data.engine_no, { shouldValidate: true });
            setValue('remarks', res.data.data.remarks, { shouldValidate: true });
            setValue('condition', res.data.data.condition, { shouldValidate: true });
            setValue('usage', res.data.data.usage, { shouldValidate: true });
            setValue('salesDate', dayjs(res.data.data.sale_date), { shouldValidate: true });
            setValue('salesValue', res.data.data.sale_value, { shouldValidate: true });
            setValue('saleTo', res.data.data.sale_to, { shouldValidate: true });
            setValue('wdvAmount', res.data.data.wdv_amount, { shouldValidate: true });
            setValue('approvedBy', res.data.data.approved_by, { shouldValidate: true });
            setValue('modeOfPayment', res.data.data.payment_mode, { shouldValidate: true });
            setValue('paymentFromBank', res.data.data.payment_from_bank, { shouldValidate: true });
            setValue('paymentToBank', res.data.data.payment_to_bank, { shouldValidate: true });
            setValue('chequeDate', dayjs(res.data.data.cheque_date), { shouldValidate: true });
            setValue('chequeNo', res.data.data.cheque_no, { shouldValidate: true });
            setValue('chequeTo', res.data.data.cheque_to, { shouldValidate: true });
            setValue('transactionDate', dayjs(res.data.data.transaction_date), { shouldValidate: true });
            setValue('transactionNo', res.data.data.transaction_no, { shouldValidate: true });
            setValue('transactionStatus', res.data.data.transaction_status, { shouldValidate: true });
            setValue('amount', res.data.data.amount, { shouldValidate: true });
            setValue('driver', res.data.data.driver, { shouldValidate: true });
            // setValue('rcBook', res.data.data.rc_book_details, { shouldValidate: true });
            // setValue('pucDetails', res.data.data.puc_details, { shouldValidate: true });
            // setValue('insuranceDetails', res.data.data.insurance_details, { shouldValidate: true });
        }
        catch(err){

            console.log(err);
            alert("something went wrong in get specific commercial vehicle");
        }
    }
    ...
    const onSubmit = handleSubmit(async (data) => {
        ...
        const driver_id = allDrivers.filter((d) => {
            if (d?.label === data?.driver) {
                return true;
            }
            else {
                return false;
        ...
        // console.log("here is our last hope : ", rcBookImage);
        ...
        try {
            ...
            // formData.append("file", rcBookImage)
                    ...
                    <Card sx={{ p: 3, width: '80%', }}>
                        ...
                        >
                            ...
                            {/* <RHFAutocomplete
                                name="vehicleType"
                                label="Vehicle Type *"
                                options={vehicleTypes.map((vehicleType) => vehicleType.label)}
                                getOptionLabel={(option) => option}
                                isOptionEqualToValue={(option, value) => option === value}
                                renderOption={(props, option) => {
                                    const { label } = vehcileTypes.filter(
                                        (vehicleType) => vehicleType.label === option
                                    )[0];
                            ...
                            <RHFAutocomplete
                                name="driver"
                                label="Driver *"
                                options={allDrivers.map((driver) => driver.label)}
                                getOptionLabel={(option) => option}
                                isOptionEqualToValue={(option, value) => option === value}
                                renderOption={(props, option) => {
                                    const { label } = allDrivers.filter(
                                        (driver) => driver.label === option
                                    )[0];
                        ...
                        </Box>
                        {
                                ...
                                <strong>Commercial Vehicle details has been saved!</strong>
                            </Alert>
                        }

                        <Stack alignItems="flex-end" sx={{ mt: 3 }}>
                            <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
                                {!currentUser ? 'Add Commercial Vehicle Details' : 'Save Changes'}
                            </LoadingButton>
                        </Stack>
                    </Card>
                </Grid>
...
EditCommercialVehicleForm.propTypes = {

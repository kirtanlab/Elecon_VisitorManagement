...
import RequisitionTableRow from "./escalated-requisition-row";
...
export default function AdminRender(){
  ...
  const [getApiCalled, setGetApiCalled] = useState(false);
    ...
    const getAllEscalatedRequisition = async() =>{
      try{
        const res = await axios.get(`http://localhost:5000/api/v1/requisition/admin/escalated/getAll`)
        console.log('res of getAllEscalatedRequisition',res?.data?.data);

...
import GrievanceActivity from "./admin/GrievanceActivity";
...
const TABLE_HEAD = [
  { id: 'title', label: 'Complaint', width: 200 },
  { id: 'grievance_type', label: 'Grievance Type', width: 200 },
  { id: 'by_whom_id', label: 'Employee', width: 200 },
  // { id: 'desc', label: 'Description' },
  { id: 'grievance_date', label: 'Date', width: 180 },
...
export default function AdminRender(){
  ...
  const [getApiCalled, setGetApiCalled] = useState(false);
    ...
    const complaintOfHod = async() =>{
      try{
        const res = await axios.get(`http://localhost:5000/api/v1/grievance`)
        console.log('res of complaintOfHod',res?.data?.data);
  ...
  function formatDate(date) {
    ...
    const tempHeaders = ["Title", "Message", "Employee Name", "Employee Email", "HOD Name", "HOD Email", "Grievance Date", "Status"];
    ...
    const URL = `http://localhost:5000/api/v1/grievance/analysis/print/all/get`
    ...
    const tabled = res?.data?.data.map((el) => {
        return {
            Title: el.title,
            Message: el.message,
            "Employee Name": el.by_whom_id.emp_name,
            "Employee Email": el.by_whom_id.email,
            "HOD Name": el.by_whom_id.hod_id.emp_name,
            "HOD Email": el.by_whom_id.hod_id.email,
            "Grievance Date": formatTimeAndDate(el.grievance_date),
            "Status": el.status,
        }
    })
    ...
    doc.save(`${formatDate(new Date())}Grievance.pdf`)
    }

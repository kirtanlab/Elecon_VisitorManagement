...
const TABLE_HEAD = [
  { id: 'title', label: 'Complaint', width: 100 },
  { id: 'grievance_type', label: 'Grievance Type', width: 100 },
  { id: 'by_whom_id', label: 'Employee', width: 100 },
  // { id: 'desc', label: 'Description' },
  { id: 'grievance_date', label: 'Date', width: 140 },
...
export default function HodRender(){
  ...
  const [getApiCalled, setGetApiCalled] = useState(false);
    ...
    const complaintOfHod = async() =>{
      try{
        const res = await axios.get(`http://localhost:5000/api/v1/grievance/hod/${user._id}`)

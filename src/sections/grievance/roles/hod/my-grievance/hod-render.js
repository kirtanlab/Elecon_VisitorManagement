...
const TABLE_HEAD = [
  { id: 'title', label: 'Complaint', width: 120},
  { id: 'grievance_type', label: 'Grievance Type', width: 120 },
  { id: 'grievance_date', label: 'Date' ,width: 100},
...
//   const table = useTable({ defaultOrderBy: 'orderNumber' });
  ...
  const [getApiCalled, setGetApiCalled] = useState(false);
    ...
    const complaintOfEmp = async() =>{
      try{
        const res = await axios.get(`http://localhost:5000/api/v1/grievance/${user._id}`)

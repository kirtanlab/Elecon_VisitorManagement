...
export default function ComplaintTableRowForEmp({ row, onViewRow }) {
  const { _id ,title, message, grievance_type, by_whom_id, grievance_date, is_resend, status,} = row;

  const theme = useTheme();
  const popover = usePopover();

  // colculation of time till complaint has fired 
  const startDate = new Date(grievance_date);
  ...
  const onResend = async() => {
    try {
      const res = await axios.post(`http://localhost:5000/api/v1/grievance/${_id}/resend`)
      toggleAddDepartmentFlag()
      alert("Grievance Resent")
    } catch (error) {
      console.log(error);
      alert("something went wrong in resend");
    }
  }

  const onEscalate = async() => {
    try {
      const res = await axios.post(`http://localhost:5000/api/v1/grievance/${_id}/escalate`)
      toggleAddDepartmentFlag()
      alert("Grievance Escalated")
    } catch (error) {
      console.log(error);
      alert("something went wrong in escalate")
    }
  }

  const onRemind = async() => {
    try {
      const res = await axios.post(`http://localhost:5000/api/v1/grievance/${_id}/remind`)
      toggleAddDepartmentFlag()
      alert("Grievance Reminded");
    } catch (error) {
      console.log(error);
      alert("something went wrong in remind")
    }
  }

...
export default function EditAssetDetailsForm({ currentUser }) {
    ...
    const getSpecificAsset = async() => {
        try{
            const URL = `http://localhost:5000/api/v1/bill/asset/${itemMasterId}`;
            ...
            setValue('itemName', res.data.data.item, { shouldValidate: true });
            setValue('vendorName', res.data.data.vendor, { shouldValidate: true });
            // setValue('monthlyLimit', res.data.data.address1, { shouldValidate: true });
            // setValue('issuedTo', res.data.data.issued_to, { shouldValidate: true });
            // setValue('assetNo', res.data.data.data_card_no, { shouldValidate: true });
            setValue('condition', res.data.data.condition, { shouldValidate: true });
            setValue('wdvAmount', res.data.data.wdv_amount ?  res.data.data.wdv_amount: 0, { shouldValidate: true });
            setValue('approvedBy', res.data.data.approved_by, { shouldValidate: true });
            // setValue('requisitionDate', res.data.data.amount, { shouldValidate: true });
            setValue('issuedDate', dayjs(res.data.data.issue_date), { shouldValidate: true });
            setValue('purchaseDate', dayjs(res.data.data.purchase_date), { shouldValidate: true });
            setValue('saleDate', dayjs(res.data.data.sale_date), { shouldValidate: true });
            setValue('saleValue', dayjs(res.data.data.sale_value), { shouldValidate: true });
            setValue('cjoNo', res.data.data.cjo_no, { shouldValidate: true });
            ...
            console.log(err);
            alert("something went wrong in specific asset")
        }
    }
    ...
    const onSubmit = handleSubmit(async (data) => {
        // alert("this is this is ");
        console.log("here in submit edit asset details");
        // const company_id = companies.filter((c) => {
        ...
        // console.log(item_id, approved_by_id, vendor_id, emp_id)
...
EditAssetDetailsForm.propTypes = {

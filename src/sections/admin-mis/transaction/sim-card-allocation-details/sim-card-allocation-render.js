...
import SimCardAllocationForm from './sim-card-allocation-form';
import SearchBar from 'src/components/search/Search-bar';
import { useAdminMISContext } from 'src/context/admin_mis_context';
import EditSimCardAllocationForm from './edit-sim-card-form';
import ShowSimCardBillDetails from './show-sim-card-bill-details';
...
export default function SimCardAllocationRender({ title, subheader, tableLabels, tableData, ...other }) {
    ...
    const renderSimCardAllocationForm = () => {
            ...
            <Card {...other}>
                ...
                <Box sx={{ p: 3, textAlign: 'right' }}>
                    ...
                    <SearchBar
                        ...
                        placeholder={`company, division, department, employee Name`}
                    />
                    <Button
                        size="small"
                        color="inherit"
                    ...
                    >
                        Add Sim Card Allocation Details
                    </Button>
                    </Stack>
                    ...
                    >
                        <Box sx={{ p: 3, borderBottom: `solid 1px ${theme.palette.divider}` }}>
                            <InputBase
                                fullWidth
                                autoFocus
                                placeholder="Add Sim Card Allocation Details"
                                value={''}
                                onChange={() => {
                                    '';
                                }}
                        ...
                        <Scrollbar sx={{ p: 3, pt: 2, height: 400 }}>{renderSimCardAllocationForm()}</Scrollbar>
                    </Dialog>
                </Box>
                        ...
                        <Table sx={{ minWidth: 960 }}>
                            ...
                            <TableBody>
                                {dataFiltered ? dataFiltered.slice(
                                    ...
                                    <SimCardAllocationRow key={row.id} row={row} />
                                )) : <LinearProgress />}
                            </TableBody>
                        </Table>
                    </Scrollbar>
...
SimCardAllocationRender.propTypes = {
...
function SimCardAllocationRow({ row }) {
    const theme = useTheme();
    ...
    const handleDelete = async(gotId) => {
        try{
            const URL = `http://localhost:5000/api/v1/bill/simcard/${gotId}`
          ...
          >
            <Box sx={{ p: 3, borderBottom: `solid 1px ${theme.palette.divider}` }}>
              <InputBase
                fullWidth
                autoFocus
                placeholder="Detail of Sim Card Bills"
                value={''}
                onChange={() => {
                  '';
                }}
...
SimCardAllocationRow.propTypes = {
    ...
    if (name) {
        inputData = inputData.filter(
          (order) =>
            ...
            order.employee.toLowerCase().indexOf(name.toLowerCase()) !== -1 
        );
      }

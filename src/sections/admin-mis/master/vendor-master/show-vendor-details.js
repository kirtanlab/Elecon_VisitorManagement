...
export default function ShowVendorDetails() {
    ...
    const getVendor = async() => {
        try{
            const URL = `http://localhost:5000/api/v1/vendor/${itemMasterId}`
            ...
            for(const key in got){
                if(key === "_id" || key === "__v" || key === "visitors" || key === "created_date" || key === "created_by" || key === "updated_date" || key === "updated_by" || key === "deleted_date" || key === "deleted_by"){
                    continue;
                }
                else{
                  if(got[key] === ""){

...
import ShowVendorDetails from './show-vendor-details';
import SearchBar from 'src/components/search/Search-bar';

// ----------------------------------------------------------------------
export default function VendorRender({
  title,
  subheader,
  tableLabels,
  tableData,
  ...
  const { formMasterFlag, setFormMasterFlag } = useAdminMISContext();
      ...
      <Card {...other}>
        <Grid container justifyContent={'space-between'}>
          ...
          >
            ...
            <Scrollbar sx={{ p: 3, pt: 2, height: 400 }}>{renderVendorForm()}</Scrollbar>
          </Dialog>
                ...
                <Table sx={{ minWidth: 960 }}>
                  ...
                  <TableBody>
                    {dataFiltered
                      ...
                      .map((row) => (
                        <VendorDetailsRow key={row.id} row={row} />
                      ))}
                  </TableBody>
                </Table>
              </Scrollbar>
...
function VendorDetailsRow({ row }) {
  ...
  const handleDelete = async () => {
    try {
      const URL = `http://localhost:5000/api/v1/vendor/${row._id}`;

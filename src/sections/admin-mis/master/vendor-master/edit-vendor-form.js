...
export default function EditVendorForm({ currentUser }) {
  ...
  const getSpecificVendor = async () => {
    try {
      ...
      setValue('vendorName', res.data.data.name, { shouldValidate: true });
      setValue('vendorcode', res.data.data.vendor_code, { shouldValidate: true });

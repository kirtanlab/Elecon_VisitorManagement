...
export default function MakeModelMasterForm({ currentUser }) {
    ...
    const MakeModelMasterSchema = Yup.object().shape({
    ...
    const defaultValues = useMemo(
        () => ({
            ...
            modelName: currentUser?.modelName || '',
        }),
        [currentUser]
    );

    const methods = useForm({
        resolver: yupResolver(MakeModelMasterSchema),
        defaultValues,
    });
    ...
    const onSubmit = handleSubmit(async (data) => {
        ...
        try {
            const URL = `http://localhost:5000/api/v1/makeAndModel`
            const res = await axios.post(URL, {
                company: company_id,
                division: division_id,
                department: department_id,
                make_and_model_name: data.modelName
            })
                ...
                <Grid container xs={12} md={10} sx={{ marginTop: 5, }} justifyContent={'center'}>
                    <Card sx={{ p: 3, width: '80%', }}>
                        ...
                        </Box>
                        {
                                ...
                                <strong>Model name has been saved!</strong>
                            </Alert>
                        }

                        <Stack alignItems="flex-end" sx={{ mt: 3 }}>
                            <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
                                {!currentUser ? 'Add Model' : 'Save Changes'}
                            </LoadingButton>
                        </Stack>
                    </Card>
                </Grid>

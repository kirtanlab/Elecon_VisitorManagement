...
import MakeModelMasterForm from './make-model-form';
import ShowMakeModelDetails from './show-make-model';
import EditMakeModelForm from './edit-make-model';
import SearchBar from 'src/components/search/Search-bar';

// ----------------------------------------------------------------------
export default function MakeModelMasterRender({
  title,
  subheader,
  tableLabels,
  tableData,
  ...
  const renderMakeModelMasterForm = () => {
    return (
      <>
        <MakeModelMasterForm />
      </>
      ...
      <Grid container justifyContent={'space-between'}>
        <CardHeader title={title} subheader={subheader} sx={{ mb: 3 }} />
          ...
          >
            ...
            <Scrollbar sx={{ p: 3, pt: 2, height: 400 }}>{renderMakeModelMasterForm()}</Scrollbar>
          </Dialog>
          <SearchBar
            filters={filters}
              ...
              <TableBody>
                {dataFiltered ? (
                  dataFiltered
                    ...
                    .map((row) => <MakeModelMasterDetailsRow key={row.id} row={row} />)
                ) : (
                  <LinearProgress />
                )}
              </TableBody>
...
MakeModelMasterRender.propTypes = {
...
function MakeModelMasterDetailsRow({ row }) {
  const theme = useTheme();
  ...
  const handleDelete = async () => {
    try {
      const URL = `http://localhost:5000/api/v1/makeAndModel/${row._id}`;
  ...
  return (
    <>
      <TableRow>
        ...
        <TableCell>
          <ListItemText
            primary={row?.make_and_model_name}
            primaryTypographyProps={{ typography: 'body2', noWrap: true }}
            secondaryTypographyProps={{
              mt: 0.5,
              component: 'span',
...
MakeModelMasterDetailsRow.propTypes = {

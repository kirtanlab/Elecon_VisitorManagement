...
import VehicleMasterForm from './vehicle-master-form';
import EditVehicleMasterForm from './edit-vehicle-master';
import ShowVehicleMasterDetails from './show-vehicle-master';
import SearchBar from 'src/components/search/Search-bar';

// ----------------------------------------------------------------------
export default function VehicleMasterRender({
  title,
  subheader,
  tableLabels,
  tableData,
      ...
      <Grid container justifyContent={'space-between'}>
        <CardHeader title={title} subheader={subheader} sx={{ mb: 3 }} />
          ...
          >
            ...
            <Scrollbar sx={{ p: 3, pt: 2, height: 400 }}>{renderVehicleMasterForm()}</Scrollbar>
          </Dialog>
          <SearchBar
            filters={filters}
              ...
              <TableBody>
                {dataFiltered ? (
                  dataFiltered
                    ...
                    .map((row) => <VehicleMasterDetailsRow key={row.id} row={row} />)
                ) : (
                  <LinearProgress />
                )}
              </TableBody>
...
function VehicleMasterDetailsRow({ row }) {
  const theme = useTheme();
  ...
  const handleDelete = async () => {
    try {
      const URL = `http://localhost:5000/api/v1/vehicle/${row._id}`;
...
VehicleMasterDetailsRow.propTypes = {

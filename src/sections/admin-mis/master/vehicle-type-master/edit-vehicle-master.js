...
export default function EditVehicleMasterForm({ currentUser }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      const URL = `http://localhost:5000/api/v1/vehicle/${itemMasterId}`;
  ...
  const getSpecificVendor = async () => {
    try {
      ...
      setValue('vehicleName', res.data.data.vehicle_name, { shouldValidate: true });
...
EditVehicleMasterForm.propTypes = {

...
import ShowItemDetails from './show-item-master';
import EditItemMasterForm from './edit-master-form';

// ----------------------------------------------------------------------
export default function ItemMasterRender({ title, subheader, tableLabels, tableData, isApiCalled, ...other }) {
      ...
      <Card {...other}>
        <Grid container justifyContent={'space-between'}>
          ...
          >
            ...
            <Scrollbar sx={{ p: 3, pt: 2, height: 400 }}>{renderItemMasterForm()}</Scrollbar>
          </Dialog>
          <SearchBar
            filters={filters}
              ...
              <TableBody>
                {dataFiltered ? (
                  dataFiltered
                    ...
                    .map((row) => <ItemMasterDetailsRow key={row.id} row={row} />)
                ) : (
                  <LinearProgress />
                )}
              </TableBody>
...
function ItemMasterDetailsRow({ row }) {
  const theme = useTheme();
  ...
  const handleDelete = async () => {
    try {
      const URL = `http://localhost:5000/api/v1/asset/${row._id}`;
...
ItemMasterDetailsRow.propTypes = {
  ...
  if (name) {
    ...
    inputData = inputData.filter(
      (order) =>
        ...
        order.asset_name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }

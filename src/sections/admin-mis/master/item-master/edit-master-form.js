...
export default function EditItemMasterForm({ currentUser }) {
  ...
  const methods = useForm({
    resolver: yupResolver(ItemMasterSchema),
    defaultValues,
  });
  ...
  const getSpecificCompany = async () => {
    try {
      ...
      setValue('itemName', res.data.data.asset_name, {
        shouldValidate: true,
      });
          ...
          <Card sx={{ p: 3, width: '80%' }}>
            ...
            {done && (
              <Alert severity="success">
                ...
                <strong>Item name has been saved!</strong>
              </Alert>
            )}

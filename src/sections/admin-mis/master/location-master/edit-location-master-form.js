...
export default function EditLocationMasterForm({ currentUser }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      const URL = `http://localhost:5000/api/v1/location/${itemMasterId}`;
  ...
  const getSpecificLocation = async () => {
    try {
      const URL = `http://localhost:5000/api/v1/location/${itemMasterId}`;
      ...
      setValue('locationName', res.data.data.location_name, { shouldValidate: true });
...
EditLocationMasterForm.propTypes = {

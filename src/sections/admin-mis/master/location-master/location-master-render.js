...
import LocationMasterForm from './location-master-form';
import ShowLocationMasterDetails from './show-location-master';
import EditLocationMasterForm from './edit-location-master-form';
import SearchBar from 'src/components/search/Search-bar';

// ----------------------------------------------------------------------
export default function LocationMasterRender({
  title,
  subheader,
  tableLabels,
  tableData,
  ...
  const renderLocationMasterForm = () => {
    return (
      <>
        <LocationMasterForm />
      </>
      ...
      <Grid container justifyContent={'space-between'}>
        <CardHeader title={title} subheader={subheader} sx={{ mb: 3 }} />
          ...
          >
            ...
            <Scrollbar sx={{ p: 3, pt: 2, height: 400 }}>{renderLocationMasterForm()}</Scrollbar>
          </Dialog>
          <SearchBar
            filters={filters}
            onFilters={handleFilters}
            placeholder="Company or Division or Department or Item."
          />
        </Grid>
              ...
              <TableBody>
                {dataFiltered ? (
                  dataFiltered
                    ...
                    .map((row) => <LocationMasterDetailsRow key={row.id} row={row} />)
                ) : (
                  <LinearProgress />
                )}
              </TableBody>
...
LocationMasterRender.propTypes = {
...
function LocationMasterDetailsRow({ row }) {
  const theme = useTheme();
  const {editMasterFlag, setEditMasterFlag, setItemMasterId, setShowMasterFlag, showMasterFlag, setDeleteMasterFlag, deleteMasterFlag} = useAdminMISContext();
  ...
  const handleDelete = async () => {
    try {
      const URL = `http://localhost:5000/api/v1/location/${row._id}`;
...
LocationMasterDetailsRow.propTypes = {

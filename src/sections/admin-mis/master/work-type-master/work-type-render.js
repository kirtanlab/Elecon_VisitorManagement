...
import WorkTypeMasterForm from './work-type-form';
import EditWorkTypeForm from './edit-work-type';
import ShowWorkTypeDetails from './show-work-type';
import SearchBar from 'src/components/search/Search-bar';

// ----------------------------------------------------------------------
export default function WorkTypeMasterRender({
  title,
  subheader,
  tableLabels,
  tableData,
  ...
  const renderWorkTypeMasterForm = () => {
    return (
      <>
        <WorkTypeMasterForm />
      </>
      ...
      <Grid container justifyContent={'space-between'}>
        <CardHeader title={title} subheader={subheader} sx={{ mb: 3 }} />
          ...
          >
            ...
            <Scrollbar sx={{ p: 3, pt: 2, height: 400 }}>{renderWorkTypeMasterForm()}</Scrollbar>
          </Dialog>
          <SearchBar
            filters={filters}
              ...
              <TableBody>
                {dataFiltered ? (
                  dataFiltered
                    ...
                    .map((row) => <WorkTypeMasterDetailsRow key={row.id} row={row} />)
                ) : (
                  <LinearProgress />
                )}
              </TableBody>
...
WorkTypeMasterRender.propTypes = {
...
function WorkTypeMasterDetailsRow({ row }) {
  const theme = useTheme();
  ...
  const handleDelete = async () => {
    try {
      const URL = `http://localhost:5000/api/v1/workType/${row._id}`;
...
WorkTypeMasterDetailsRow.propTypes = {

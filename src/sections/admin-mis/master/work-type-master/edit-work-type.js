...
export default function EditWorkTypeForm({ currentUser }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      const URL = `http://localhost:5000/api/v1/workType/${itemMasterId}`;
  ...
  const getSpecificVendor = async () => {
    try {
      ...
      setValue('workType', res.data.data.work_type, { shouldValidate: true });

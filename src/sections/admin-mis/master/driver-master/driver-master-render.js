...
import DriverMasterForm from './driver-master-form';
import EditDriverForm from './driver-master-edit';
import ShowDriverDetails from './show-driver-details';
import SearchBar from 'src/components/search/Search-bar';


// ----------------------------------------------------------------------
export default function DriverMasterRender({ title, subheader, tableLabels, tableData, isApiCalled, ...other }) {
      ...
      <Grid container justifyContent={'space-between'}>
        <CardHeader title={title} subheader={subheader} sx={{ mb: 3 }} />
          ...
          >
            ...
            <Scrollbar sx={{ p: 3, pt: 2, height: 400 }}>{renderDriverMasterForm()}</Scrollbar>
          </Dialog>
          <SearchBar
            filters={filters}
              ...
              <Table sx={{ minWidth: 960 }}>
                ...
                <TableBody>
                  {dataFiltered ? dataFiltered.slice(
                    ...
                    <DriverMasterDetailsRow key={row.id} row={row} />
                  )) : <LinearProgress />}
                </TableBody>
              </Table>
            </Scrollbar>
...
function DriverMasterDetailsRow({ row }) {
  const theme = useTheme();
  ...
  const handleDelete = async () => {
    try {
      const URL = `http://localhost:5000/api/v1/driver/${row._id}`;
...
DriverMasterDetailsRow.propTypes = {

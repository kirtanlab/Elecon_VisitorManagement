...
export default function VisitorDetailsGateUser({
  title,
  subheader,
  tableLabels,
  tableData,
...
function VisitorDetailsRow({ row }) {
  ...
  const getNameOfEmp = async () => {
    const res = await axios.get(
      `http://localhost:5000/api/v1/employee/${row.to_whom_id}`
    );
  ...
  const {
    addAccFlagGateUser,
    setAddAccFlagGateUser,
    setAddAccFlag,
    showAccFlagGateUser,
    setShowAccFlagGateUser,
    editVisitorFlagGateUser,
    setEditVisitorFlagGateUser,
    setVisitorId,
    visitorId,
    outGateUser,
    setOutGateUser,
    ...
    <>
      ...
      >
        ...
        <Dialog
          ...
          onClose={() => {
            setEditVisitorFlagGateUser(false);
          }}
          transitionDuration={{
      ...
      <InDialog open={confirmIn.value} onClose={confirmIn.onFalse} row={row} />
